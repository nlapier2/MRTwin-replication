...
def mr_ivw(beta_e, se_e, beta_o, se_o):
	...
	res = model.fit()
	est = res.params[0]
	se = res.bse[0]/min(1,np.sqrt(res.scale))
	t = est/se
...
def compute_statistic(beta_e, se_e, beta_o, se_o, genotypes, outcome_trait, mr_method, mr_est=None):
	...
	stat = -sum(np.square(pred_outcomes - outcome_trait))
	# stat = stats.pearsonr(pred_outcomes, outcome_trait)[0] ** 2
	return stat
...
def compute_statistic_sib(beta_e, se_e, beta_o, se_o, genotypes, outcome_trait, mr_method, mr_est=None):
	...
	for i in range(len(genotypes)):
		...
		break  # use only first sib per set to determine statistic
